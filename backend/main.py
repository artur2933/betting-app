...
<body>
    ...
    <div class="main-content">
        ...
        <div id="generator" class="page">
            ...
            <div id="ticket-result" class="card" style="display:none; border: 1px solid #00ff88;"></div>
        </div>

        <div id="history" class="page">
            <div class="header"><h2>História Tiketov</h2></div>
            <div class="card">
                <div class="ticket-row"><span>Arsenal vs Chelsea</span><span style="color: #00ff88;">VÝHRA ✅</span></div>
                <div class="ticket-row"><span>Real Madrid vs Barcelona</span><span style="color: red;">PREHRA ❌</span></div>
            </div>
        </div>
        
         <div id="settings" class="page">
            <div class="header"><h2>Nastavenia</h2></div>
            <div class="card"><p>Nastavenia budú dostupné čoskoro.</p></div>
        </div>
    </div>
    ...
    <script>
        ...
        async function generujTiket() {
            ...
            try {
                ...
                if (data.length === 0) { div.innerHTML = '<p>Žiadne vhodné zápasy.</p>'; return; }
                let html = '<h3>🔥 Dnešný AI Výber:</h3>';
                let totalOdds = 1;
                data.forEach(z => {
                    totalOdds *= z.kurz;
                    html += `<div class="ticket-row"><div><b>${z.domaci} vs ${z.hostia}</b><br><small style="color:#aaa">${z.dovod}</small></div><div class="odds">${z.kurz}</div></div>`;
                });
                ...
                div.innerHTML = html;
            } catch(e) { div.innerHTML = '<p style="color:red">Chyba spojenia.</p>'; }
        }
    </script>
</body>
</html>
...
# Oprava chyby: Rozdelenie funkcie na viac riadkov
def get_db():
