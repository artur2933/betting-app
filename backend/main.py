...
# Jednoduchá cache, aby sme neplatili za každé načítanie stránky
CACHE = {"data": [], "last_update": 0}

def get_live_data():
    # Ak sú dáta mladšie ako 1 hodina, vráti cache (šetrí API limit)
    if time.time() - CACHE["last_update"] < 3600 and CACHE["data"]:
        return CACHE["data"]

    # Ak nie je kľúč, vráti demo
    if API_KEY == "VLOZ_SVOJ_API_KLUC_SEM":
        return generate_demo_data()

    try:
        # Sťahujeme reálne dáta
        ...
        for item in data[:15]: 
            try:
                ...
                # AI Logika (Určenie tipu)
                risk = 1
                tip = "1"
                dovera = 80
                analyza = "Zápas s vyrovnanými kurzami."

                if o1 < 1.55:
                    risk = 1; tip = "1"; dovera = random.randint(85, 95)
                    analyza = f"{home} je doma veľmi silný. Hostia z {away} sa trápia."
                elif o2 < 1.55:
                    risk = 1; tip = "2"; dovera = random.randint(85, 95)
                    analyza = f"{away} má lepšiu formu a kvalitnejší káder."
                elif o1 < 2.15:
                    risk = 2; tip = "1"; dovera = random.randint(65, 80)
                    analyza = "Domáce prostredie bude rozhodujúce. Hodnotný kurz."
                elif o2 < 2.15:
                    risk = 2; tip = "2"; dovera = random.randint(65, 80)
                    analyza = "Hostia sú miernym favoritom, očakávame tesný výsledok."
                else:
                    risk = 3; tip = "X"; dovera = random.randint(40, 60)
                    analyza = "Vysoké kurzy na oboch stranách. Remíza je pravdepodobná."

                # Falošné stats pre "PRO" vzhľad
                stats = {
                    "utok_domaci": int(100/o1) if o1 > 1 else 90,
                    "utok_hostia": int(100/o2) if o2 > 1 else 90,
...
def generate_demo_data():
    return [{"domaci": "DEMO REŽIM", "hostia": "VLOŽ KĽÚČ", "kurz": 1.00, "tip": "Info", "risk": 1, "liga": "System", "dovera": 0, "stats": {"utok_domaci":0, "utok_hostia":0, "forma_domaci": "-", "forma_hostia": "-", "zranenia": "-"}, "analyza_text": "Vlož API kľúč do main.py na GitHube."}]
...
# --- HTML FRONTEND (Bez prihlasovania) ---
html_content = """
<!DOCTYPE html>
<html lang="sk">
<head>
    ...
    <style>
        ...
        .btn-bet { background: transparent; border: 1px solid var(--primary); color: var(--primary); padding: 8px 20px; border-radius: 5px; cursor: pointer; font-weight: bold; margin-top: 10px; width: 100%; transition:0.2s; }
        ...
        .ac-bar-home { background: var(--primary); height: 100%; } .ac-tip-box { background: #1a222e; border: 1px solid #2c3e50; border-radius: 8px; padding: 15px; display: flex; justify-content: space-between; align-items: center; border-left: 4px solid var(--primary); margin-top: 20px; }
...
<body>
    ...
    <script>
        ...
        function initChart() {
            ...
            if(history.length === 0) dataPoints = [1000, 1000, 1000, 1000, 1000]; // Start line
        ...
        function resetApp() {
            if(confirm("Naozaj chceš vymazať celú históriu a začať odznova?")) {
                localStorage.clear();
                location.reload();
            }
        }
        ...
        function evaluateTickets() {
            ...
            history.forEach(t => {
                if(t.status === "PENDING") {
                    let won = Math.random() < 0.60; // 60% win rate simulácia
