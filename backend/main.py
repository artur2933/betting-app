...
<head>
    ...
    <style>
        ...
        .btn-analyze { background: linear-gradient(45deg, #45a29e, #66fcf1); border: none; padding: 15px 40px; font-size: 18px; font-weight: bold; color: #0b0c10; border-radius: 30px; cursor: pointer; box-shadow: 0 0 20px rgba(102, 252, 241, 0.3); transition: transform 0.2s; display: block; margin: 0 auto 30px auto; }
        .btn-analyze:hover { transform: scale(1.05); }
        .match-card { background: #1f2833; border-radius: 10px; margin-bottom: 25px; overflow: hidden; border: 1px solid #333; box-shadow: 0 5px 15px rgba(0,0,0,0.5); animation: slideUp 0.5s ease; }
    ...
    <script>
        ...
        async function generujTiket() {
            ...
            try {
                ...
                out.innerHTML = html;
            } catch(e) { load.style.display = 'none'; alert("Chyba spojenia so serverom."); }
        }
    </script>
</body>
</html>
...
# 3. BACKEND (MUSÍ TU BYŤ NA KONCI)
def get_db():
...
def generuj_denny_tiket(db: Session = Depends(get_db)):
    profesionalne_data = [
        ...
        {"domaci": "Lazio", "hostia": "Porto", "kurz": 1.75, "tip": "BTTS (Obaja gól)", "stats": {"goly_priemer": "1.8 - 1.9", "xg_data": "1.20 vs 1.35", "utok_domaci": 65, "utok_hostia": 70}, "analyza_titulek": "Otvorený zápas", "analyza_text": "Lazio doma vždy skóruje."},
